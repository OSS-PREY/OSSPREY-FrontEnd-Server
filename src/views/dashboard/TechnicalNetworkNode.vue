<template>
  ...
  // Computed property to get the selected developer's name from the store
  const selectedNodeName = computed(() => projectStore.selectedDeveloper);
  ...
  // Watch for changes in selectedDeveloper to reset commitLinks and fetch new data
  watch(
    () => projectStore.selectedDeveloper,
    (newDeveloper, oldDeveloper) => {
      console.log(`Selected Developer changed from ${oldDeveloper} to ${newDeveloper}`);
      if (newDeveloper && newDeveloper !== oldDeveloper) {
        commitLinks.value = []; // Clear the existing commit links
        openDialog(); // Fetch new commit links for the selected developer
      }
    }
  );
  </script>
